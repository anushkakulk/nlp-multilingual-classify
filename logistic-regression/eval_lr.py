...
def evaluate(language, df, vectorizer, lr_model): 
    X_combined = combine_sentences(df)
    y = df["label"].values
    
    X_vector = vectorizer.transform(X_combined)
    y_pred = lr_model.predict(X_vector)
...
def main():
    ...
    parser.add_argument("--vectorizer", default="models/vectorizer.joblib")
    ...
    if args.en_test:
        ...
        evaluate("English Test", en_df, vectorizer, lr_model)

    if args.es_test:
        es_df = pd.read_csv(args.es_test, on_bad_lines="skip", engine="python")
        evaluate("Spanish Test", es_df, vectorizer, lr_model)

    if args.zh_test:
        zh_df = pd.read_csv(args.zh_test, on_bad_lines="skip", engine="python")
        evaluate("Chinese Test", zh_df, vectorizer, lr_model)
